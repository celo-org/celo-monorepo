...
describe('signatures', () => {
  it('should sign appropriately with the messageHash', () => {
    ...
    const signature = signMessageHash(message, pKey, address)
    const serializedSig = serializeSignature(signature)
    parseSignatureForMessageHash(message, serializedSig, address)
  })
