...
export function* sendInvite(
  ...
  try {
    ...
    CeloAnalytics.track(AnalyticsEvents.invite_success)
  } catch (e) {
    CeloAnalytics.track(AnalyticsEvents.invite_error, { error: e.message })
...
export function* redeemInviteSaga({ inviteCode }: RedeemInviteAction) {
  ...
  if (result === true) {
    ...
    CeloAnalytics.track(AnalyticsEvents.redeem_invite_timed_out)
    yield put(redeemInviteFailure())
...
export function* doRedeemInvite(inviteCode: string) {
  try {
    ...
    if (tempAccountBalanceWei.isLessThanOrEqualTo(0)) {
      CeloAnalytics.track(AnalyticsEvents.redeem_invite_failed, { context: 'Empty invite' })
    ...
    CeloAnalytics.track(AnalyticsEvents.redeem_invite_success)
    return true
  } catch (e) {
    Logger.error(TAG + '@doRedeemInvite', 'Failed to redeem invite', e)
    CeloAnalytics.track(AnalyticsEvents.redeem_invite_failed, { error: e.message })
...
export function* skipInvite() {
  ...
  try {
    ...
    CeloAnalytics.track(AnalyticsEvents.invite_skip_complete)
    navigateHome()
  } catch (e) {
    Logger.error(TAG, 'Failed to skip invite', e)
    CeloAnalytics.track(AnalyticsEvents.invite_skip_failed, { error: e.message })
