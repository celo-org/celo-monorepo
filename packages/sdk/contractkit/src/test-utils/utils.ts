...
const EPOCH = 100
export const currentEpochNumber = async (web3: Web3, epochSize: number = EPOCH) => {
  const blockNumber = await web3.eth.getBlockNumber()

  return getEpochNumberOfBlock(blockNumber, epochSize)
}

const getFirstBlockNumberForEpoch = (epochNumber: number, epochSize: number = EPOCH) => {
  if (epochNumber === 0) {
...
const getEpochNumberOfBlock = (blockNumber: number, epochSize: number = EPOCH) => {
...
export const mineToNextEpoch = async (web3: Web3, epochSize: number = EPOCH) => {
  const blockNumber = await web3.eth.getBlockNumber()
