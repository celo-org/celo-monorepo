...
import { ErrorMessages, FileKeystore, InMemoryKeystore } from './keystores'
...
describe('KeystoreBase functionality via InMemoryKeystore (mock)', () => {
  ...
  describe('checks with an empty keystore', () => {
    ...
    xit('imports keystore (can decrypt, can list addresses)', async () => {
    ...
    xit('gets empty address map', async () => {
      expect(await keystore.getAddressMap()).toEqual({})
    })
    xit('fails when address is not in the keystore', async () => {
...
describe('FileKeystore tests', () => {
  const testWorkdir = path.join(__dirname, 'wallet-keystore-workdir')
  beforeAll(() => {
    mkdirSync(testWorkdir, { recursive: true })
  })

  afterAll(() => {
    rmdirSync(testWorkdir, { recursive: true })
  })
