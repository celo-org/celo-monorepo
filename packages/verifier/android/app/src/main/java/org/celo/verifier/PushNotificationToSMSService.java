...
public class PushNotificationToSMSService extends FirebaseMessagingService {
        ...
        new Transaction.Handler() {
          ...
          public Transaction.Result doTransaction(MutableData currentData) {
            ...
            if (message == null) {
              Log.d(
                TAG + "@runTransaction",
                "Message object is null, local cache may not be ready."
              );
            ...
            if (
              message.messageState != MessageState.DISPATCHING.ordinal() ||
          ...
          public void onComplete(DatabaseError error, boolean committed, DataSnapshot snapshot) {
            ...
            if (error != null) {
              Log.e(
                TAG + "@runTransaction",
                "Message transaction error: " + error.getMessage() + " : " + error.getDetails()
              );
              return;
            }
            ...
            if (
              message.messageState != MessageState.ASSIGNED.ordinal() || message.verifierId != uid
            ) {
              Log.e(
                TAG + "@runTransaction",
                "Message tx commited but it is not assigned to this verifier. This should never happen."
              );
              return;
            }
            ...
            Log.d(
              TAG + "@runTransaction",
              "Message claimed successfully. Will attempt to send SMS"
            );
  ...
  private Boolean canSendSMS() {
    return (
      ...
      ContextCompat.checkSelfPermission(getApplicationContext(), permission.SEND_SMS) ==
  ...
  private void sendSMS(String phoneNumber, String message, final String messageId) {
    ...
    try {
      ...
      Intent sendSMS = new Intent(
        SMSStatusBroadcastReceiver.SMS_SENT_INTENT,
        null,
        this,
        SMSStatusBroadcastReceiver.class
      );
      ...
      smsLogIntent.putParcelableArrayListExtra(
        EXTRA_SMS_LOGS,
        (ArrayList<? extends Parcelable>) smsLogs
      );
  ...
  public static String getSMSServiceStatus(Context context) {
    SharedPreferences prefs = context.getSharedPreferences(
      PushNotificationToSMSService.USER_PREF_STORE,
      Context.MODE_PRIVATE
    );
    String serviceStatus = prefs.getString(
      PushNotificationToSMSService.USER_PREF_SERVICE_STATUS,
      "ON"
    );
