...
export default class AuthorizeVoteSigner extends BaseCommand {
  ...
  static examples = [
    'authorize-vote-signer --from 0x5409ED021D9299bf6814279A6A1411A7e866A631 --signer 0x6ecbe1db9ef729cbe972c83fb886247691fb6beb --pop 0x1b9fca4bbb5bfb1dbe69ef1cddbd9b4202dcb6b134c5170611e1e36ecfa468d7b46c85328d504934fce6c2a1571603a50ae224d2b32685e84d4d1a1eebad8452eb',
  ]
  ...
  async run() {
    ...
    const sender = isRevoked
      ? await releaseGoldWrapper.getReleaseOwner()
      : await releaseGoldWrapper.getBeneficiary()
    const tx = await releaseGoldWrapper.authorizeVoteSigner(flags.signer, sig)
    await displaySendTx('authorizeVoterTx', tx, { from: sender })
  }
}
