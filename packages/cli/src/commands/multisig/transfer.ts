...
export default class MultiSigTransfer extends BaseCommand {
  ...
  static flags = {
    ...
    transferFrom: flags.boolean({ description: 'Perform transferFrom instead of transfer in the ERC-20 interface' }),
    sender: Flags.address({ description: 'Identify sender if performing transferFrom' }),
    from: Flags.address({ required: true, description: 'Account transferring value to the recipient' }),
  }
