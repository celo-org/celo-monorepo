...
export default class Authorize extends BaseCommand {
  ...
  async run() {
    ...
    if (res.flags.role === 'vote') {
      tx = await vestingFactoryInstance.authorizeVoteSigner(res.flags.signer, sig)
    } else if (res.flags.role === 'validator') {
      tx = await vestingFactoryInstance.authorizeValidatorSigner(res.flags.signer, sig)
    } else if (res.flags.role === 'attestation') {
      tx = await vestingFactoryInstance.authorizeAttestationSigner(res.flags.signer, sig)
    } else {
