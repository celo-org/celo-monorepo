...
export function stripHexEncoding(hexString: string) {
	return hexString.substring(0, 2) === "0x"
...
export async function currentEpochNumber(
	web3: Web3,
	epochSize: number = EPOCH,
) {
...
export function getEpochNumberOfBlock(
	blockNumber: number,
	epochSize: number = EPOCH,
) {
...
export const assertThrowsAsync = async (promise: any, errorMessage = "") => {
	let failed = false;
	try {
		await promise;
	} catch (_) {
...
export async function assertTransactionRevertWithReason(
	promise: any,
	...
	try {
		...
		const msg: string =
			expectedRevertReason === ""
				? `Expected "StatusError", got ${error} instead`
				: `Expected ${expectedRevertReason}, got ${error} instead`;
		assert(revertFound, msg);
	}
}

export async function assertTransactionRevertWithoutReason(
	promise: any,
	errorMessage = "",
) {
...
export async function assertRevert(promise: any, errorMessage = "") {
	// Only ever tested with ganache.
...
async function isPortOpen(host: string, port: number) {
	return (
		(await execCmd("nc", ["-z", host, port.toString()], { silent: true })) === 0
	);
}
	...
	for (const contractList of proxiedContracts) {
		for (const contractName of contractList.contracts) {
			const contract = await getContract(
				contractName,
				"contract",
				contractList.__contractPackage,
			);
			const proxy: ProxyInstance = await getContract(
				contractName,
				"proxy",
				contractList.__contractPackage,
			);
...
export const assertContractsRegistered = async (getContract: ContratGetter) => {
	...
	for (const proxyPackage of hasEntryInRegistry) {
		for (const contractName of proxyPackage.contracts) {
			const contract: Truffle.ContractInstance = await getContract(
				contractName,
				proxyPackage,
			);
			assert.strictEqual(
				contract.address.toLowerCase(),
				(
					await registry.getAddressFor(soliditySha3(contractName))
				).toLowerCase(),
...
export const assertRegistryAddressesSet = async (
	getContract: ContratGetter,
) => {
	const registry: RegistryInstance = await getContract("Registry");
	for (const contractName of usesRegistry) {
		const contract: UsingRegistryInstance = await getContract(
			contractName,
			MENTO_PACKAGE,
		);
		assert.strictEqual(
...
export const assertContractsOwnedByMultiSig = async (getContract: any) => {
	const multiSigAddress = (await getContract("MultiSig", "proxiedContract"))
		.address;
	for (const contractList of ownedContracts) {
		for (const contractName of contractList.contracts) {
			const contractOwner: string = await (
				await getContract(
					contractName,
					"proxiedContract",
					contractList.__contractPackage,
			...
			assert.strictEqual(
				contractOwner,
				multiSigAddress,
				contractName + " is not owned by the MultiSig",
			);
		}
	}
	...
	for (const contractList of proxiedContracts) {
		for (const contractName of contractList.contracts) {
			const proxyOwner = await (
				await getContract(contractName, "proxy", contractList.__contractPackage)
			)._getOwner();
			assert.strictEqual(
				proxyOwner,
				multiSigAddress,
				contractName + "Proxy is not owned by the MultiSig",
			);
		}
	}
};
...
export function assertLogMatches(
	...
	assertObjectWithBNEqual(
		log.args,
		args,
		(arg) => `Event ${event}, arg: ${arg} do not match`,
	);
}
...
export function assertBNArrayEqual(actualArray: any[], expectedArray: any[]) {
	assert(Array.isArray(actualArray), `Actual is not an array`);
	assert(Array.isArray(expectedArray), `Expected is not an array`);
	assert(
		actualArray.length === expectedArray.length,
		`Different array sizes; actual: ${actualArray.length} expected: ${expectedArray.length}`,
	);
	assert(
		actualArray.every((actualValue) => isNumber(actualValue)) &&
			expectedArray.every((expectedValue) => isNumber(expectedValue)),
...
export function assertEqualBN(
	...
	assert(
		...
		`expected ${expected.toString(10)} and got ${actual.toString(10)}. ${
			msg || ""
		}`,
	);
}
...
export function assertAlmostEqualBN(
	...
	assert(
		...
		`expected ${expected.toString(10)} to be within ${margin.toString(
			10,
		)} of ${actual.toString(10)}. ${msg || ""}`,
	);
}
...
export function assertEqualDpBN(
	...
	assert(
		...
		`expected ${expectedDp.toString()} and got ${valueDp.toString()}. ${
			msg || ""
		}`,
	);
}
...
const ownedContracts = [
	{
		contracts: ["Attestations", "Escrow", "Registry", "SortedOracles"],
	},
	{
		contracts: ["Reserve", "Exchange", "StableToken"],
		__contractPackage: MENTO_PACKAGE,
	},
];
...
export const assertSameAddress = (
	value: string,
	expected: string,
	msg?: string,
) => {
...
export function createMatcher<A>(
	assertFn: (value: A, expected: A, msg?: string) => void,
) {
...
export async function assumeOwnershipWithTruffle(
	contractsToOwn: string[],
	to: string,
	dequeuedIndex = 0,
	contractPackage?: ContractPackage,
) {
	const governance: GovernanceInstance = await getDeployedProxiedContract(
		"Governance",
		artifacts,
	);
	const lockedGold: LockedGoldInstance = await getDeployedProxiedContract(
		"LockedGold",
		artifacts,
	);
	const multiSig: GovernanceApproverMultiSigInstance =
		await getDeployedProxiedContract("GovernanceApproverMultiSig", artifacts);
	...
	const proposalTransactions = await Promise.all(
		contractsToOwn.map(async (contractName: string) => {
			...
			const contractAddress = (
				await getDeployedProxiedContract(contractName, artifactsInstance)
			).address;
