...
export async function verifyProxyStorageProofFoundry(
  proofLookup: ProofLookup,
  proxy: string,
  owner: string
) {
  const proof = await proofLookup.getProof(proxy, [OWNER_POSITION, IMPLEMENTATION_POSITION])
