...
contract MultiMetaTransactionWallet is ICeloVersionedContract, InitializableV2, Ownable {
   ...
   */
  function initialize(address[] calldata _signers) external {
  ...
  function executeMetaTransaction(
    ...
    require(isSigner[_signer], "Invalid meta-transaction signer");
    nonce = nonce.add(1);
    ...
    returns (bytes memory)
  {
    ...
    require(isSigner[msg.sender] || msg.sender == owner(), "Invalid transaction sender multi");
    bytes memory returnData = ExternalCall.execute(destination, value, data);
