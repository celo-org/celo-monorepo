...
import "@celo-contracts-8/common/EpochManagerEnablerMockMigrations.sol"; // imported  just to force the compilation // TODO remove me

import "@celo-contracts-8/common/EpochManager.sol";
import { MigrationsHelper } from "@migrations-sol/MigrationsHelper.sol";

contract Migration is Script, UsingRegistry, MigrationsConstants, MigrationsHelper {
  ...
  function deployProxiedContract(
    ...
    return deployProxiedContract(contractName, contractName, initializeCalldata);
  }
  ...
  function deployProxiedContract(
    string memory contractName,
    string memory contractImplementationName,
    bytes memory initializeCalldata
  ) public returns (address proxyAddress) {
    console.log("Deploying: ", contractName);
    ...
    setImplementationOnProxy(proxy, contractImplementationName, initializeCalldata);
    addToRegistry(contractName, address(proxy));
  ...
  function migrateEpochManagerEnabler() public {
    deployProxiedContract(
      "EpochManagerEnabler",
      "EpochManagerEnablerMockMigrations",
