...
module.exports = deploymentForCoreContract<FeeCurrencyDirectoryInstance>(
  ...
  async (feeCurrencyDirectory: FeeCurrencyDirectoryInstance, _web3: Web3, networkName: string) => {
    ...
    const feeCurrencyDirectoryInstance = await getDeployedProxiedContract<FeeCurrencyDirectoryInstance>('FeeCurrencyDirectory', artifacts08)
    const nentoFeeCurrencyAdapterV1Instance = await getDeployedProxiedContract<MentoFeeCurrencyAdapterV1Instance>('MentoFeeCurrencyAdapterV1', artifacts08)
    ...
    for (const token of ['StableToken', 'StableTokenEUR', 'StableTokenBRL']) {
      ...
      console.log("setting currency config for", token, "with address", stableToken.address, "and adapter address", nentoFeeCurrencyAdapterV1Instance.address, "on network", networkName);
      await feeCurrencyDirectoryInstance.setCurrencyConfig(stableToken.address, nentoFeeCurrencyAdapterV1Instance.address, 1)
    }
