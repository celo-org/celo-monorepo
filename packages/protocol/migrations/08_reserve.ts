...
const initializeArgs = async (): Promise<
  [string, number, string, number, number, string[], string[], string, string, string[], number[]]
> => {
  ...
  return [
    ...
    config.reserve.collateralAssetAddresses,
    config.reserve.collateralAssetDailySpendingRatios,
  ]
}
