...
The function logs a structured `USAT_TOPUP_NEEDED` line (`jsonPayload.alert =
"usat_topup"`) before paying when the balance cannot cover the run or would
drop below `MIN_BALANCE_USAT` afterwards, and after paying when wallets were
skipped for lack of funds or the remaining balance is below the threshold. A
Cloud Monitoring log-match alert policy ("USAT rewards wallet needs a top-up")
emails the notification channel(s) with the wallet address and the numbers.
Add a recipient: create an email notification channel and attach it to the
policy in Cloud Monitoring → Alerting.
