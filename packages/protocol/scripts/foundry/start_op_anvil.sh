...
if nc -z localhost $ANVIL_OP_PORT; then
  echo "Port already used"
  kill $(lsof -t -i:$ANVIL_OP_PORT)
  sleep 5
...
# alternatively:
# ANVIL_PID=`lsof -i tcp:9545 | tail -n 1 | awk '{print $2}'`
export ANVIL_PID=$!

echo "Waiting Anvil to launch on $ANVIL_OP_PORT..."
while ! nc -z localhost $ANVIL_OP_PORT; do
  sleep 0.1 # wait for 1/10 of the second before check again
done

# enabled logging
cast rpc anvil_setLoggingEnabled true --rpc-url $ANVIL_OP_RPC_URL
