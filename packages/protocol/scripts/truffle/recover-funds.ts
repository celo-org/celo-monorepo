...
async function handleGrant(config: ReleaseGoldConfig, currGrant: number) {
  ...
  // 1. The MultiSigProxy was deployed
  ...
  // 2. The MultiSigProxy implementation was set
  ...
  // 3. The ReleaseGoldProxy was deployed at A with key deployer
  ...
  // 4. The ReleaseGoldProxy implementation was set
  // 5. The proxy was funded
  // 6. Calling _setAndInitializeImplementation again failed
  ...
  try {
    ...
    console.log('Somthing went wrong! Recovering funds...')
    // recover funds
    console.log(1)
    const recoveryMultiSig = await retryTx(ReleaseGoldMultiSig.new, [{ from: fromAddress }])
    console.log(3)
    // 2. The MultiSigProxy implementation was set
    ...
    const releaseGold = await ReleaseGold.at(releaseGoldProxy.address)
    await releaseGold.transfer(fromAddress, totalValue.minus(startGold), { from: fromAddress })
