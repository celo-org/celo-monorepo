...
//         exchangeRegistryId = keccak256(abi.encodePacked("Exchange"));
//     }
...
//      */
//     function burn(uint256 value) external updateInflationFactor override(ICeloToken, IStableToken) returns (bool) {
//         require(
//             msg.sender == registry.getAddressForOrDie(getExchangeRegistryId()) ||
//                 msg.sender == registry.getAddressFor(GRANDA_MENTO_REGISTRY_ID),
...
//      */
//     function balanceOf(address accountOwner) external view override(IERC20, IStableToken) returns (uint256) {
...
//         if (block.timestamp < inflationState.factorLastUpdated.add(inflationState.updatePeriod)) {
...
//         uint256 timesToApplyInflation = block.timestamp.sub(inflationState.factorLastUpdated).div(inflationState.updatePeriod);
