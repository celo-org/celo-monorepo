...
contract BaseTest is Test {
  ...
  function setUpContracts() private {
    ...
    governance = new GovernanceForTest();
    governance.initialize(
      ...
      DEPOSIT, // minDeposit
      60 * 60, // queryExpiry
      10 * 60, // dequeueFrequency
      REFERENDUM_STAGE_DURATION,
      1 * 60, // executionStageDuration
...
contract Initialize is BaseTest {
...
contract SetApprover is BaseTest {
  ...
  function testRevert_WhenCalledByNotOnwer() public {
...
contract SetMinDeposit is BaseTest {
...
contract SetConcurrentProposals is BaseTest {
...
contract SetQueueExpiry is BaseTest {
  ...
  function testRevertIf_Unchanged() public {
    ...
    vm.prank(accOwner);
    governance.setQueueExpiry(60 * 60);
  }
...
contract SetDequeueFrequency is BaseTest {
  ...
  function testRevertIf_Unchanged() public {
    ...
    vm.prank(accOwner);
    governance.setDequeueFrequency(10 * 60);
  }
...
contract SetReferendumStageDuration is BaseTest {
...
contract SetExecutionStageDuration is BaseTest {
  ...
  function testRevertIf_Unchanged() public {
    vm.expectRevert("Duration unchanged");
    vm.prank(accOwner);
    governance.setExecutionStageDuration(1 * 60);
  }
...
contract SetParticipationFloor is BaseTest {
  ...
  function test_SetsValue() public {
    ...
    (uint256 baseline, uint256 baselineFloor, uint256 baselineUpdateFactor, uint256 baselineQuorumFactor) = governance
      .getParticipationParameters();
...
contract SetBaselineUpdateFactor is BaseTest {
  ...
  function test_SetsValue() public {
    ...
    (uint256 baseline, uint256 baselineFloor, uint256 baselineUpdateFactor, uint256 baselineQuorumFactor) = governance
      .getParticipationParameters();
    assertEq(baselineUpdateFactor, NEW_VALUE);
  }
...
contract SetBaselineQuorumFactor is BaseTest {
  ...
  function test_SetsValue() public {
    ...
    (uint256 baseline, uint256 baselineFloor, uint256 baselineUpdateFactor, uint256 baselineQuorumFactor) = governance
      .getParticipationParameters();
    assertEq(baselineQuorumFactor, NEW_VALUE);
  }
...
contract SetConstitution is BaseTest {
  ...
  function testRevertsIf_ThresholdIsNotGreatedThanMajority() public {
...
contract Propose is BaseTest {
  ...
  function check_emitsEvents(Proposal memory proposal) private {
    vm.expectEmit(true, true, true, true);
...
contract Upvote is BaseTest {
  ...
  function test_SortsUpvotedProposalToFronOfQueue_WhenItWasAtTheEnd() public {
  ...
  function test_DeuqeueQueuedProposals_whenItHasBeenMoreThanDequeueFrequencySinceLastDequeue()
    public
  {
    uint256 originalLastDequeue = governance.lastDequeue();
    uint256 newProposalId = makeValidProposal();
  ...
  function testRevertIf_UpvotingAProposalThatWillBeDequeuede_whenItHasBeenMoreThanDequeueFrequencySinceLastDequeue()
    public
  {
    uint256 originalLastDequeue = governance.lastDequeue();
    uint256 newProposalId = makeValidProposal();
...
contract RevokeUpvote is BaseTest {
  event ProposalExpired(uint256 indexed proposalId);
...
contract Approve is BaseTest {
  ...
  function test_returnTrue_whenInReferendumStage() public {
  ...
  function test_NotDeleteProposal_whenInReferendumStage() public {
    // Dequeue the other proposal.
    uint256 newProposalId = makeValidProposal();
    vm.prank(accApprover);
    governance.approve(proposalId, INDEX);
...
contract RevokeVotes is BaseTest {
...
contract VoteWhenProposalIsApproved is BaseTest {
  ...
  function test_SetVotersVoteRecord() public {
    vm.prank(accVoter);
    governance.vote(proposalId, 0, Proposals.VoteValue.Yes);
    (uint256 recordProposalId, uint256 deprecietedVoteValue, uint256 deprecietedWeight, uint256 yesVotesRecord, uint256 noVotesRecord, uint256 abstainVotesRecord) = governance
...
contract VoteWhenProposalIsApprovedAndHaveSigner is BaseTest {
  event ProposalVotedV2(
    uint256 indexed proposalId,
    address indexed account,
    uint256 yesVotes,
...
contract VoteWhenProposalIsNotApproved is BaseTest {
  event ProposalVotedV2(
    uint256 indexed proposalId,
    address indexed account,
    uint256 yesVotes,
...
contract VoteWhenVotingOnDifferentProposalWithSameIndex is BaseTest {
...
contract VotePartiallyWhenProposalIsApproved is BaseTest {
  event ProposalVotedV2(
    uint256 indexed proposalId,
    address indexed account,
    uint256 yesVotes,
...
contract VotePartiallyWhenProposalIsApprovedAndHaveSigner is BaseTest {
  event ProposalVotedV2(
    uint256 indexed proposalId,
    address indexed account,
    uint256 yesVotes,
...
contract VotePartiallyWhenProposalIsNotApproved is BaseTest {
  event ProposalVotedV2(
    uint256 indexed proposalId,
    address indexed account,
    uint256 yesVotes,
...
contract VotePartiallyWhenVotingOnDifferentProposalWithSameIndex is BaseTest {
  function test_IgnoreVotesFromPreviousProposal() public {
    uint256 proposalId1 = makeValidProposal();
...
contract ApproveHotfix is BaseTest {
...
contract WhitelistHotfix is BaseTest {
...
contract HotfixWhitelistValidatorTally is BaseTest {
  bytes32 constant HOTFIX_HASH = bytes32(uint256(0x123456789));
...
contract IsHotfixPassing is BaseTest {
  bytes32 constant HOTFIX_HASH = bytes32(uint256(0x123456789));
...
contract PrepareHotfix is BaseTest {
  ...
  function test_markHotfixRecordPreparedEpoch_whenHotfixIsPassing() public {
    ...
    governance.whitelistHotfix(HOTFIX_HASH);
    governance.prepareHotfix(HOTFIX_HASH);
    (bool approved, bool executed, uint256 preparedEpoch) = governance.getHotfixRecord(HOTFIX_HASH);
...
contract ExecuteHotfix is BaseTest {
  ...
  function test_markHotfixAsExecuted_WhenApprovedAndPreparedForCurrentEpoch() public {
    approveAndPrepareHotfix();
    executeHotfixTx();
    (bool _, bool executed, ) = governance.getHotfixRecord(hotfixHash);
    assertTrue(executed);
  }
...
contract IsProposalPassing is BaseTest {
  uint256 proposalId;
  address accSndVoter;
...
contract DequeueProposalsIfReady is BaseTest {
...
contract GetProposalStage is BaseTest {
...
contract GetAmountOfGoldUsedForVoting is BaseTest {
...
contract RemoveVotesWhenRevokingDelegatedVotes is BaseTest {
  uint256[] proposalIds;
  ...
  function assertVoteRecord(
    ...
    (uint256 proposalId, uint256 deprecietedVoteValue, uint256 deprecietedWeight, uint256 yes, uint256 no, uint256 abstain) = governance
      .getVoteRecord(accVoter, index);
