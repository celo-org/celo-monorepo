...
function revokeActive(
    ...
    require (election.getActiveVoteUnitsForGroupByAccount(group,address(this)) <= election.getActiveVoteUnitsForGroup(group));
  ...
  function releaseTime() public view returns (uint256) {
    return releaseSchedule.releaseStartTime.add(
  ...
  function getTotalPendingWithdrawals(address a) public view returns (uint256) {
   ...
   function withdrawalPossible(uint256 amount) external nonReentrant onlyBeneficiary {
    ...
    beneficiary/* @certora Beneficiary.nothing() */.transfer(amount);
