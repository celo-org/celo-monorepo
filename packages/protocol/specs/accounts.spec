...
invariant address_cant_be_both_delegate_of_two_address(address x, address y, address d) 
	...
	(!sinvoke _getAttestationSigner(y)==d && !sinvoke _getVoteSigner(y)==d && !sinvoke _getValidatorSigner(y)==d)

invariant address_can_authorize_two_address(address x, address d1, address d2) 
	(x!=d1 && x!=d2 && d1!=d2 && sinvoke isAccount(x) && sinvoke _getAttestationSigner(x)==d1 && sinvoke _getVoteSigner(x)==d2) => 
