...
export class AzureKeyVaultClient {
  ...
  public static getAddressFromPublicKey(publicKey: BN): Promise<string> {
    const ecPublicKey = Nat.from(publicKey)
    const publicKeyString = '0x' + ecPublicKey.encode('hex', false).slice(2)
    const publicHash = Account.keccak256(publicKeyString)
    return Account.toChecksum('0x' + publicHash.slice(-40))
  }
