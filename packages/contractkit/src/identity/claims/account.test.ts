...
testWithGanache('Account claims', (web3) => {
  ...
  it("can't claim itself", async () => {
    const metadata = IdentityMetadataWrapper.fromEmpty(address)
    await expect(
      metadata.addClaim(createAccountClaim(address), NativeSigner(kit.communication.sign, address))
  ...
  describe('verifying', () => {
    ...
    beforeEach(async () => {
      ...
      await metadata.addClaim(claim, NativeSigner(kit.communication.sign, address))
    })
