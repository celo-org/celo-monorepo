...
export class PnpSignAction implements Action<SignMessageRequest> {
  ...
  public async perform(
    ...
    const hasQuota = true;  // TODO - check cached data on this.
        ...
        if (!hasQuota) {
             this.io.sendFailure(
               ...
               403, 
               session.response, 
               "some quota status -- don't hit DB or node")
             return
        }
        const key: Key = {
          version:
    ...
    // Do this quota update stuff stuff async at the end, only if signiture success.
    // Update the "cached" value at the end.
    // async updateQuota (don't block)    
    ...
    //X       const { sufficient } = await this.quota.checkAndUpdateQuotaStatus(
    //X        quotaStatus,
    //X         session,
    // X        trx
    //       )
    // X      if (!sufficient) {
    //  X       this.io.sendFailure(WarningMessage.EXCEEDED_QUOTA, 403, session.response, quotaStatus)
    //   X      return
    //    X   }
    // }
