...
export const getDistributedBlindedSalt = functions
  ...
  .https.onRequest(async (req, res) => {
    logger.info({ req }, 'Begin getDistributedBlindedSalt request')
    await handleGetBlindedMessageSig(req, res)
    logger.info({ res }, 'Sent getDistributedBlindedSalt response')
  })
...
export const getBlindedMessageSig = functions
  .region('us-central1', 'europe-west3')
  .https.onRequest(async (req, res) => {
    logger.info({ req }, 'Received getBlindedMessageSig request')
    await handleGetBlindedMessageSig(req, res)
    logger.info({ res }, 'Sent getBlindedMessageSig response')
  })
...
export const getContactMatches = functions
  .region('us-central1', 'europe-west3')
  .https.onRequest(async (req, res) => {
    logger.info({ req }, 'Received getContactMatches request')
    await handleGetContactMatches(req, res)
    logger.info({ res }, 'Sent getContactMatches response')
  })
