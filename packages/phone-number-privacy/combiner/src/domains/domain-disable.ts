...
// import {
//   authenticateUser, CombinerEndpoints, DomainRestrictedSignatureRequest, ErrorMessage,
...
export async function handleDomainDisableReq(_request: Request, _response: Response) {
  // TODO(Alec)
}
...
// async function handleDisableDomain(
//   request: Request,
//   response: Response
// ) {
...
//       return
//     }
//     if (!(await authenticatDisableDomainRequest(request, logger))) {
//       respondWithError(response, 401, WarningMessage.UNAUTHENTICATED_USER, logger)
//       return
//     }
//     logger.debug('Requesting signatures')
//     await disableDomain(request, response)
//   } catch (err) {
//     logger.error('Unknown error in handleDisableDomain')
//     logger.error(err)
