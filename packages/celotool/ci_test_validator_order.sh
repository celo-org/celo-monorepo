...
if [ "${1}" == "checkout" ]; then
    ...
    ../../node_modules/.bin/mocha --bail -r ts-node/register src/e2e-tests/validator_order_tests.ts --branch ${BRANCH_TO_TEST}
elif [ "${1}" == "local" ]; then
    export GETH_DIR="${2}"
    echo "Testing using local geth dir ${GETH_DIR}..."
    ../../node_modules/.bin/mocha --bail -r ts-node/register src/e2e-tests/validator_order_tests.ts --localgeth ${GETH_DIR}
fi
