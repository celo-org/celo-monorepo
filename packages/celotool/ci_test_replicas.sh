...
if [ "${1}" == "checkout" ]; then
    ...
    ./node_modules/.bin/mocha --node-option loader=ts-node/esm src/e2e-tests/replica_tests.ts --branch ${BRANCH_TO_TEST}
elif [ "${1}" == "local" ]; then
    export GETH_DIR="${2}"
    echo "Testing using local geth dir ${GETH_DIR}..."
    ./node_modules/.bin/mocha --node-option loader=ts-node/esm src/e2e-tests/replica_tests.ts --localgeth ${GETH_DIR}
fi
