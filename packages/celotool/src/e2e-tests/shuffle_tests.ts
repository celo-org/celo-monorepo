...
describe('governance tests', () => {
  ...
  describe('Validators', () => {
    ...
    it('requests an attestation', async function(this: any) {
      ...
      const latestBlock = (await contractKit.web3.eth.getBlock('latest')).number

      while ((await contractKit.web3.eth.getBlock('latest')).number < latestBlock + 25) {
        await sleep(2)
      ...
      const blocks = await Promise.all(
        Array.from(Array(25).keys()).map(async (num) =>
          contractKit.web3.eth.getBlock(num + latestBlock)
        )
      )
      const miners = blocks.map((block) => block.miner)
