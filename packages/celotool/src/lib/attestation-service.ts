...
async function helmParameters(celoEnv: string) {
  ...
  return [
    ...
    ...setHelmArray('attestation_service.sms_providers', fetchEnvOrFallback(envVar.SMS_PROVIDERS, '').split(',')),
    `--set attestation_service.sms_providers_randomized=${fetchEnv(envVar.SMS_PROVIDERS_RANDOMIZED)}`,
