...
export async function downloadArtifacts(celoEnv: string) {
  let baseCmd = `yarn --cwd ../protocol run artifacts:download -n ${celoEnv}`
  console.info(`Downloading artifacts for ${celoEnv}`)
  if (isProduction()) {
...
export async function uploadArtifacts(celoEnv: string, checkOrPromptIfStagingOrProduction = true) {
  ...
  let baseCmd = `yarn --cwd ../protocol run artifacts:upload -n ${celoEnv}`
  if (isProduction()) {
    baseCmd += ` -b contract_artifacts_production`
  }
  console.info(`Uploading artifacts for ${celoEnv}`)
  try {
    await execCmd(baseCmd)
  } catch (error) {
