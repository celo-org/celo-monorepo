...
```bash
# On the Attestation machine
docker run -v $PWD:/root/.celo --rm -it $CELO_IMAGE --nousb account proof-of-possession $CELO_ATTESTATION_SIGNER_ADDRESS $CELO_VALIDATOR_RG_ADDRESS
```
...
```bash
# On your local machine
export CELO_ATTESTATION_SIGNER_SIGNATURE=<YOUR-ATTESTATION-SIGNER-SIGNATURE>
export CELO_ATTESTATION_SIGNER_ADDRESS=<YOUR-ATTESTATION-SIGNER-ADDRESS>
celocli releasegold:authorize --from $CELO_VALIDATOR_RG_ADDRESS --role attestation --signature 0x$CELO_ATTESTATION_SIGNER_SIGNATURE --signer $CELO_ATTESTATION_SIGNER_ADDRESS
```
...
```bash
# On the Attestation machine
echo <YOUR-ATTESTATION-SIGNER-PASSWORD> > .password
docker run --name celo-attestations -it --restart always -p 127.0.0.1:8545:8545 -v $PWD:/root/.celo $CELO_IMAGE --verbosity 3 --networkid $NETWORK_ID --nousb --syncmode full --rpc --rpcaddr 0.0.0.0 --rpcapi eth,net,web3,debug,admin --unlock $CELO_ATTESTATION_SIGNER_ADDRESS --password /root/.celo/.password --bootnodes $BOOTNODE_ENODES
```
...
Run the following commands on your local machine where `$CELO_VALIDATOR_RG_ADDRESS` is unlocked.

```bash
# On your local machine
celocli releasegold:create-metadata ./metadata.json --from $CELO_VALIDATOR_RG_ADDRESS
```
...
```bash
# On your local machine
celocli releasegold:claim-attestation-service-url ./metadata.json --url $ATTESTATION_SERVICE_URL --from $CELO_VALIDATOR_RG_ADDRESS
```
...
```bash
# On your local machine
celocli releasegold:claim-account ./metadata.json --address $CELO_VALIDATOR_GROUP_RG_ADDRESS --from $CELO_VALIDATOR_RG_ADDRESS
```
...
```bash
# On your local machine
celocli releasegold:register-metadata --url <METADATA_URL> --from $CELO_VALIDATOR_RG_ADDRESS
```
...
```bash
# On your local machine
celocli releasegold:get-metadata $CELO_VALIDATOR_ADDRESS
```
...
```bash
# On your local machine
celocli releasegold:test-attestation-service --from $CELO_VALIDATOR_RG_ADDRESS --phoneNumber <YOUR-PHONE-NUMBER-E164-FORMAT> --message <YOUR_MESSAGE>
```
...
```bash
# On your local machine
celocli releasegold:create-metadata ./group-metadata.json --from 0x$CELO_VALIDATOR_GROUP_RG_ADDRESS
celocli releasegold:claim-account ./group-metadata.json --address 0x$CELO_VALIDATOR_RG_ADDRESS --from 0x$CELO_VALIDATOR_GROUP_RG_ADDRESS
# Upload group-metadata.json
celocli releasegold:register-metadata --url <GROUP_METADATA_URL> --from $CELO_VALIDATOR_GROUP_RG_ADDRESS
```

Now when you run `celocli releasegold:get-metadata $CELO_VALIDATOR_RG_ADDRESS`, you should see your claim for the group account to be verified. By now, you should have setup your Validator account appropriately. Note that you need to add these claims for any other addresses that are yours to calculate your score for the leaderboard appropriately.
