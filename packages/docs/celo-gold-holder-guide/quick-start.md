...
If you are an early self-custodying holder of Celo Gold on the Celo [Release Candidate](../getting-started/rc1/) network, this guide will help you access your account and do the setup necessary to earn rewards on those funds.
...
You will now need to point the Celo CLI to a node that is synchronized with the [Release Candidate](../getting-started/rc1/) network. There are two options:
...
* **Local Celo Blockchain node**: You can run a full node on your local machine which will communicate with other nodes and cryptographically verify all data it receives. Since this approach does not require you to trust the network, it is most secure.
  ...
  To do this, follow the tutorial for [running a full node](../getting-started/rc1/running-a-full-node-in-rc1.md) \(and make sure to pass `--nousb`\).
