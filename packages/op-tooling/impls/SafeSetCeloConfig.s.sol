...
contract SafeSetCeloConfig is Script {
  // This script requires running with --root and the following env vars:
  // CONFIG_PROXY (required) - address of the CeloSuperchainConfig proxy
  // CONFIG_IMPL (required) - address of the new CeloSuperchainConfig implementation
  ...
  function readEnv() internal view returns (EnvConfig memory) {
    return
      EnvConfig(
        vm.envAddress("CONFIG_PROXY"),
        vm.envAddress("CONFIG_IMPL"),
