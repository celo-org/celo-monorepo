...
for ID in "${CLABS_SIGNERS[@]}"; do
  ...
  eval "CLABS_SIGNER_${ID}_SIG_V2=\${CLABS_SIGNER_${ID}__SIG_V2}"
  eval "CLABS_SIGNER_${ID}_SIG_V3=\${CLABS_SIGNER_${ID}__SIG_V3}"
done

for ID in "${COUNCIL_SIGNERS[@]}"; do
  eval "COUNCIL_SIGNER_${ID}=\${COUNCIL_SIGNER_${ID}__ADDRESS}"
  eval "COUNCIL_SIGNER_${ID}_SIG_V2=\${COUNCIL_SIGNER_${ID}__SIG_V2}"
  eval "COUNCIL_SIGNER_${ID}_SIG_V3=\${COUNCIL_SIGNER_${ID}__SIG_V3}"
done

for ID in "${GC_D1C_SIGNERS[@]}"; do
  eval "GC_D1C_SIGNER_${ID}=\${GC_D1C_SIGNER_${ID}__ADDRESS}"
  eval "GC_D1C_SIGNER_${ID}_SIG_V2=\${GC_D1C_SIGNER_${ID}__SIG_V2}"
  eval "GC_D1C_SIGNER_${ID}_SIG_V3=\${GC_D1C_SIGNER_${ID}__SIG_V3}"
done
...
# ...bytes4(keccak256(abi.encodePacked("upgrade((address,address,bytes32)[],bool)"))) = 0xa4589780
  ...
  GC_SIG="0x"
  for ID in "${GC_D1C_SIGNERS[@]}"; do
    sig_var="GC_D1C_SIGNER_${ID}_SIG_${VERSION_UPPER}"
    sig_val="${!sig_var}"
    GC_SIG+="${sig_val:2}"
  done
  ...
  COUNCIL_SIG="0x"
  for ID in "${COUNCIL_SIGNERS[@]}"; do
    sig_var="COUNCIL_SIGNER_${ID}_SIG_${VERSION_UPPER}"
    sig_val="${!sig_var}"
  ...
  CLABS_SIG="0x"
  for ID in "${CLABS_SIGNERS[@]}"; do
    sig_var="CLABS_SIGNER_${ID}_SIG_${VERSION_UPPER}"
    sig_val="${!sig_var}"
    CLABS_SIG+="${sig_val:2}"
  done
