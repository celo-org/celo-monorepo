...
data "aws_security_group" "validator" {
    name = "celo-validator"
}
...
resource "aws_instance" "celo_validator" {
    ...
    vpc_security_group_ids      = [data.aws_security_group.validator.id]
    ...
    user_data                       = join("\n", [
        ...
        templatefile("${path.module}/../startup-scripts/run-validator-node.sh", {
            ...
            validator_signer_private_key                = each.value.signer_private_key
