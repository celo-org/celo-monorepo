...
const run = async () => {
  const deployer = await contractKit.contracts.getMetaTransactionWalletDeployer(
    MTW_DEPLOYER_ADDRESS
  )
  const mtwWalletAddress = await deployer.getWallet(WALLET_ADDRESS)
  const stable = await contractKit.contracts.getStableToken()
  const receipt = await stable
    .transfer(
      mtwWalletAddress,
      (await stable.balanceOf(WALLET_ADDRESS))
        // @ts-ignore
        .toFixed()
    )
