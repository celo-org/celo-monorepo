...
export CELO_IMAGE="us.gcr.io/celo-testnet/geth@sha256"
export CELO_NETWORK="4bc97381db0bb81b7a3e473bb61d447c90be165834316d3f75bc34d7db718b39"
...
export CELO_IMAGE=${3:-"us.gcr.io/celo-testnet/geth@sha256"}
export CELO_NETWORK=${4:-"4bc97381db0bb81b7a3e473bb61d447c90be165834316d3f75bc34d7db718b39"}
export NETWORK_ID=${5:-"1101"}
export DEFAULT_PASSWORD=${5:-"1234"}
...
remove_containers () {
    ...
    docker rm $(docker ps -a|grep celo-proxy|cut -d' ' -f 1)    
    docker kill $(docker ps -a|grep celo-validator|cut -d' ' -f 1) > /dev/null 2>&1 
    docker rm $(docker ps -a|grep celo-validator|cut -d' ' -f 1)
}
...
if [[ $COMMAND == *"help"* ]]; then
    ...
    echo -e "Options:"
    echo -e "$0 <COMMAND> <DATA_DIR> <CELO_IMAGE> <CELO_NETWORK> <NETWORK_ID> <PASSWORD>"
...
if [[ $COMMAND == *"pull"* ]]; then
    ...
    echo -e "* Downloading docker image from $CELO_IMAGE:$CELO_NETWORK"
...
if [[ $COMMAND == *"accounts"* ]]; then
    ...
    export CELO_VALIDATOR_ADDRESS=$(docker run -v $PWD/validator:/root/.celo --entrypoint /bin/sh -it $CELO_IMAGE:$CELO_NETWORK -c " printf '%s\n' $DEFAULT_PASSWORD $DEFAULT_PASSWORD | geth account new " |tail -1| cut -d'{' -f 2| tr -cd "[:alnum:]\n" )
    export CELO_VALIDATOR_GROUP_ADDRESS=$(docker run -v $PWD/validator:/root/.celo --entrypoint /bin/sh -it $CELO_IMAGE:$CELO_NETWORK -c " printf '%s\n' $DEFAULT_PASSWORD $DEFAULT_PASSWORD | geth account new " |tail -1| cut -d'{' -f 2| tr -cd "[:alnum:]\n" )
    export CELO_PROXY_ADDRESS=$(docker run -v $PWD/proxy:/root/.celo --entrypoint /bin/sh -it $CELO_IMAGE:$CELO_NETWORK -c " printf '%s\n' $DEFAULT_PASSWORD $DEFAULT_PASSWORD | geth account new " |tail -1| cut -d'{' -f 2| tr -cd "[:alnum:]\n" )
    ...
    export CELO_VALIDATOR_POP=$(docker run -v $PWD/validator:/root/.celo --entrypoint /bin/sh -it $CELO_IMAGE:$CELO_NETWORK -c " printf '%s\n' $DEFAULT_PASSWORD | geth account proof-of-possession $CELO_VALIDATOR_ADDRESS "| tail -1| cut -d' ' -f 3| tr -cd "[:alnum:]\n" )
